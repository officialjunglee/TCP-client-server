...
int main() {
    ...
    server_addr.sin_port = htons(49152); //Use the port for mirror
    server_addr.sin_addr.s_addr = INADDR_ANY;  //use any address to accept the request from

    if (bind(server_socket, (struct sockaddr *)&server_addr, sizeof(server_addr)) == -1) { //bind to listen to the incoming connections
